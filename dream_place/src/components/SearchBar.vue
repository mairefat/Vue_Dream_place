          ...
          <div class="flex-1">
            <div class="relative">
              <div class="flex items-center border rounded p-2 bg-gray-200">
                ...
                <!-- Dropdown List -->
                <ul
                  v-show="showDropdownGuests"
                  class="absolute z-10 w-full bg-white border rounded top-full"
                >
                  <li
                    v-for="guestOption in guestOptions"
                    :key="guestOption"
                    class="py-1 px-3 cursor-pointer hover:bg-gray-100"
                    @click="selectGuest(guestOption)"
                  >
                    {{ guestOption }}
                  </li>
                </ul>
          ...
          <div class="flex-1 bg-gray-200">
            <div class="relative">
              <div class="flex items-center border rounded p-3">
                ...
                <!-- Dropdown List -->
                <ul
                  v-show="showDropdownRooms"
                  class="absolute z-10 w-full bg-white border rounded top-full"
                >
                  <li
                    v-for="roomOption in roomsOptions"
                    :key="roomOption"
                    class="py-1 px-3 cursor-pointer hover:bg-gray-100"
                    @click="selectRooms(roomOption)"
                  >
                    {{ roomOption }}
                  </li>
                </ul>
...
const openArrivalDatePicker = () => {
  if (!arrivalDatePickerInstance.value) {
    arrivalDatePickerInstance.value = flatpickr(arrivalDateInput.value, {
      dateFormat: "m/d/Y",
...
const openDepartureDatePicker = () => {
  if (!departureDatePickerInstance.value) {
    departureDatePickerInstance.value = flatpickr(departureDateInput.value, {
      dateFormat: "m/d/Y",
      minDate: arrivalDate.value,
      onClose: (selectedDates) => {
        if (selectedDates && selectedDates.length > 0) {
...
const handleClickOutside = (event) => {
  const dropdownElement = document.querySelector(".flex");
...
const handleSearch = async () => {
  ...
  //  API request to get destination information
  ...
  try {
    ...
    if (destinationResponse.data && destinationResponse.data.status === true) {
      const dest_id = destinationResponse.data.data[0].dest_id;
      ...
      console.error(
        "Error fetching destination data. Response:",
